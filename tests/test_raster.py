...
def submit(req):
    return requests.post(SUBMIT_URL,
                  json.dumps(req),
...
req = { 'user_id': 'my_id', 'user_email_address': 'invalid', 'source': 'invalid', 'dst_format': 'invalid' }
r = submit(req)
assert(r.status_code == 400)
resp = json.loads(r.text)
assert(resp['status'] == 'ERROR')
...
req = { 'user_id': 'my_id', 'user_email_address': 'foo@bar.com', 'source': 'invalid', 'dst_format': 'invalid' }
r = submit(req)
assert(r.status_code == 400)
resp = json.loads(r.text)
assert(resp['status'] == 'ERROR')
...
r = requests.put(END_POINT_ROOT + "/jobs/inexisting_task_id",
...
r = requests.put(resp['possible_requests']['abort']['url'],
