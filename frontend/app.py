...
def submit():
    ...
    if ds is None:
        return make_error(
            _("Cannot open source '%s'. Error message : %s")
            % (source, gdal.GetLastErrorMsg()),
            req,
        )
    ...
    if is_raster:
        if "DCAP_RASTER" not in drv_md:
            return make_error(
                _("Driver '%s' has no raster capabilities") % dst_format_name
            )
        if "DCAP_CREATE" not in drv_md and "DCAP_CREATECOPY" not in drv_md:
            return make_error(
                _("Driver '%s' has no creation capabilities") % dst_format_name
            )
    else:
        if drv.GetMetadataItem("DCAP_VECTOR") is None:
            return make_error(
                _("Driver '%s' has no vector capabilities") % dst_format_name
            )
    ...
    if is_raster:
        if "layer" in req:
            return make_error(
                _("'%s' is present, but unexpected for a raster source") % "layer"
            )
        ...
        for k in req:
            if k.startswith("img_"):
                return make_error(
                    _("'%s' is present, but unexpected for a raster source") % k
                )
    ...
    if "footprint" in req:
        ...
        if not is_valid:
            ...
            return make_error(
                _("%s is not a valid footprint: %s") % (str(footprint), validity_error),
                req,
            )
        ...
        if footprint_is_json and footprint_src_srs.GetAuthorityCode(None) != "4326":
            return make_error(
                _("As footprint is a JSon geometry, footprint_srs should be EPSG:4326")
            )
    ...
    if "img_resampling_method" in req:
        ...
        if img_resampling_method not in (
            ...
            return make_error(
                _(
                    "img_resampling_method value should be one "
